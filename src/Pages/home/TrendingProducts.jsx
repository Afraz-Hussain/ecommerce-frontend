...
const TrendingProducts = () => {
  // FIXED: Added /backend prefix to match your server.js routes
  const API_BASE = `${import.meta.env.VITE_API_URL}/backend`;
  const FETCH_URL = `${API_BASE}/product/viewallproducts`;
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {products.slice(index, index + itemsPerPage).map((p) => (
            <div key={p._id} className="group flex flex-col p-0 overflow-hidden">
              <div className="relative h-60 w-full overflow-hidden mb-6 rounded-lg bg-gray-50">
                <img
                  // FIXED: Ensure image path matches your backend (might need /uploads/)
              ...
              <p className="text-sm text-gray-500 mb-3">{p.category?.cateName || 'Premium'}</p>
